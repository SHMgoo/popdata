 ...
 ** 
 \*/
        ...
        for (i = 0, ii = valuesy.length; i < ii; i++) {
            if (opts.shade) {
                shades.push(paper.path().attr({ stroke: "none", fill: colors[i], opacity: opts.nostroke ? 1 : 0.3 }));
            }
 ...
 \*/
        ...
        for (i = 0, ii = valuesy.length; i < ii; i++) {
            ...
            for (var j = 0, jj = valuesy[i].length; j < jj; j++) {
                ...
                (Raphael.is(sym, "array") ? sym[j] : sym) && symset.push(paper[Raphael.is(sym, "array") ? sym[j] : sym](X, Y, (opts.width || 2) * 3).attr({ fill: colors[i], stroke: "none", opacity: 0.8 }));
