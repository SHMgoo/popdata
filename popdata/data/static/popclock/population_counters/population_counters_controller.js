    ...
    PopCountersViewController.prototype = {
        createViews: function(singleProgressBarWidth, componentProgressBarWidth, componentProgressBarHeight, digitWrapperHeight, digitImageWidth, digitImageHeight, digitImagePaths)
        {
            ...
            $.when(gettingUSJSON, gettingWorldJSON).then( function ( usData, worldData ) {
                // US
                this.usPop = usData.us.population;
        ...
        createWidgetViews: function(singleProgressBarWidth, componentProgressBarWidth, componentProgressBarHeight, digitWrapperHeight, digitImageWidth, digitImageHeight, digitImagePaths)
        {
            /*
             * singleProgressBarWidth - no longer used, as pop counter progress replaced with odometer
             * componentProgressBarWidth - the width of each progress for the rates area - todo: most likely can be deprecated as progress bars are now HTML not SVG
            ...
            $.when(gettingUSJSON, gettingWorldJSON).then( function ( usData, worldData ) {
                ...
                //Requirements call for U.S. Population at 12:02 AM EDT.
                //$('#us_pop_widget').val(addCommas(usData.us.midnight_pop)); 
                 $('#us_pop_widget').text(addCommas(usData.us.midnight_pop)); 

                //Requirements call for World Population at 12:02 AM EDT.
                //$('#world_pop_widget').val(addCommas(worldData.world.pop_midnight)); 
                 $('#world_pop_widget').text(addCommas(worldData.world.pop_midnight));                           
...
//console.log('population_rate = '+data.population_rate);
//console.log('rate_interval = '+data.rate_interval);
            ...
            gettingUSJSON.done( function( data ) {
                var table = {
                    ...
                    data: [
                            ...
                            [config.components.us.net_gain_label.replace('%@', (Math.round(1/data.us.population_rate))).replace(/<[^>]+>/g, '')],        
                            ['Date: ' + data.us.date]
                    ]
                };
                csv.push(table);
                ...
                table.data.push(['']); 
                table.data.push(['Top 10 Most Populous Countries (July 1, 2025)']);
                table.data.push(['Rank,  Country, Population']); 
                table.data.push(['']); 

                //Populate the table with the Top 10 data
                //var rowsTop10 = Object.entries(config.components.world_rates.tables[0].rows);

                var obj = config.components.world_rates.tables[0].rows;
//console.log('obj='+obj);
//console.log('obj keys='+Object.keys(obj));
                var rowsKeys = Object.keys(obj);
                var rowsTop10 = Object.keys(obj).map(function(e) { return obj[e] }) ;
//console.log('rowsTop10='+rowsTop10);

                for ( var i = 0; i < rowsTop10.length; i++)  {
                    var rank = i + 1;
                    //console.log('i='+i+' rank = '+rank+' country='+rowsTop10[i]);
                    //console.log('i='+i+' rank = '+rank+' key = '+rowsKeys[i]+' country='+rowsTop10[i]);
                    table.data.push([rank+','+rowsKeys[i]+','+rowsTop10[i]]);
                }
