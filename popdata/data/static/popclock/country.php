...
//var_dump($_GET);
    ...
    // FIPS in the world popclock context should be a two character string.
    // If it's not a two character string
    $FIPS = preg_match("/^[a-zA-Z]{2}$/",$_GET['FIPS']) === 1 ? $_GET['FIPS'] : null;
    // $FIPS = is_numeric($FIPS) ? $FIPS : "00";
    $filename = 'data/data.json';
    $world;
    
    if(isset($_GET['worldpopembed'])){
        $worldpopembeded = htmlspecialchars($_GET['worldpopembed']);
    }
    
    $_SERVER[REQUEST_URI] = htmlspecialchars($_SERVER[REQUEST_URI]);    

    /* The intent is to intercept a suspicious call to this script, and 
     * if found, reload the main page. 
     */
...
?>
            <script>
                country_script = '<?php echo $script; ?>';  
                window.location.replace(country_script);
            </script>
<?
          exit();
       }
...
function generateRandomString($length=6){
  ...
  return htmlspecialchars($randomString);
...
<head>
    ...
    <script charset="utf-8" src="./js/d3.svg.census.js"></script>
...
$(document).ready(function(){
    ...
    $('#world-select').selectize({create: true,
    sortField: 'text'});
...
var pop_format_y_axis2 = function(value){
  ...
  return result;
   ...
   function round(number){
    ...
    var export_data = {
                         get : "EXPALL2025,IMPALL2025,EXPALL2016"
    ...
    var trade_data = {
                         get : "EXPMANF2025, IMPMANF2025, EXPMANF2024, IMPMANF2024, EXPMANF2023, IMPMANF2023, EXPMANF2022, IMPMANF2022, EXPMANF2021, IMPMANF2021, EXPMANF2020,IMPMANF2020,EXPMANF2019,IMPMANF2019,EXPMANF2018,IMPMANF2018,EXPMANF2017,IMPMANF2017,EXPMANF2016,IMPMANF2016"
                        ,key : key
                        ,SCHEDULE : "<?php echo $schedule; ?>"
                    };
    var naics_data = {
    ...
    $.ajax({
        ...
        success: function ( returnedData, textStatus, jqXHR ) {
            ...
            if (returnedData) {
               ...
               $('*[data-comp-us-population]').html( us_pop_format(342620143)).parent().css('visibility', 'visible')/*.addClass("fade")*/;
               ...
               // $('*[data-comp-us-male-ratio]').html( (male/ female*100).toFixed(1) ).parent().css('visibility', 'visible')/*.addClass("fade")*/;
                $('*[data-comp-us-male-ratio]').html( (98.0).toFixed(1) ).parent().css('visibility', 'visible')/*.addClass("fade")*/;
        ...
        success: function ( returnedData, textStatus, jqXHR ) {
            if (returnedData) {            		
                ...
                returnedData.forEach(function(d,i){                	
                    ...
                    //Temporary tweak to get around last couple of weeks
                    // in 2018.
                    //n = n.setFullYear(2019);
/* This sets the bubble date on the World 'Current and Projected Population'
 * graph. It should advance automatically as we roll into a new year.
 */

                    n = date_parse(new Date().getFullYear().toString());
                    //n = date_parse((new Date().getFullYear() - 1).toString());
                    n = n.setFullYear(2026);
                    cur_y = n;					
                    var _dVal;
                ...
                var p = d3.svg.census.multi_line()
                    .margin({top: 20, right: 40, bottom: 40, left: 50})
                      .height(300)
                    .width(500)
                    .xDomain([date_parse('2020'), date_parse('2060')])
                    .displayMarkers(true)
    ...
    $.when( $.ajax({ //use when to group trade and naics data calls together for error handling.
        ...
        success: function ( returnedData, textStatus, jqXHR ) {
            ...
            if (returnedData) {
                var  exp_15 = +returnedData[1][returnedData[0].indexOf('EXPALL2025')] || 0
                    ,imp_15 = +returnedData[1][returnedData[0].indexOf('IMPALL2025')] || 0
                    ,exp_06 = +returnedData[1][returnedData[0].indexOf('EXPALL2016')] || 0
                ;
    ...
    $.ajax({
        ...
        success: function ( returnedData, textStatus, jqXHR ) {
            if (returnedData) {
				console.log(url['trade'],trade_data);
                ...
                console.log('trade_data error, no data returned from ajax call');
                API_trade_error++;
            }
        },
        error : function(xhr, errorText, errorMessage) { //If something is wrong with the apiData_trade.php call
...
console.log('returned data='+returnedData);
                ...
                API_naics_error++; 
                console.log('no data returned from naics ajax call'); 
            }
        },
...
$.ajax({
    ...
    error : function(xhr, errorText, errorMessage) {
       //If something is wrong with the apiData_trade.php call
        alert('We were unable to retrieve Country data in a timely manner.' 
            + '\nPlease try again in a bit.');
    }
    ...
    <div class="row">
        ...
        <input value='<iframe src="<?php echo "http://$_SERVER[HTTP_HOST]$_SERVER[PHP_SELF]?worldpopembed=" . $randString ;?>" width="1008" height="950" frameBorder="0" allowtransparency="true"></iframe>' />
    </div>
    <div class="row">
        <a href="http://twitter.com/share?"><img src="./images/world/twitter.png"/></a>
        <a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo "http://$_SERVER[HTTP_HOST]$_SERVER[PHP_SELF]";?>"><img src="./images/world/facebook.png"/></a>
        <a href="https://www.pinterest.com/pin/create/button/?url=<?php echo "http://$_SERVER[HTTP_HOST]$_SERVER[PHP_SELF]";?>"
    ...
    <div id="content-wrapper">
        <?php if ( strlen($FIPS) > 0  ) :  // fips country code is set ?>
            <div id="country-select"> 
                ...
                <span id="label">Select a different country</span>
                <a class="country-back" href="<?php echo _APPROOT_;?>world">back button</a>
        ...
        <span>Demographic data as of July 1, 2026, economic data for 2025 (<a href="#world-footer">source</a>)</span>

                <a target="_NEW" href="../country_print.php?FIPS=<?php echo $FIPS;?>" class="print">Print</a>
        ...
        <div class="data-container">
            <div class="data-cell" id="basic-facts">
                ...
                <div class="data-cell trade" style="background-image: url('./images/world/Icon-ChangeIn_2x.png');">
                    <p>Change in exports from U.S. for 2016 to 2025</p>        
            ...
            <div class="data-cell" id="closer-look">
                ...
                <figure>
                    <div class="inset-bg">
                        <img class="region-inset" src="<?php echo $country['CIA_Map_Region_URL'] ?>"/>
                    </div>
                    <img class="country" src="<?php echo $country['CIA_Map_Country_URL'];?>"/>
  ...
  <div class="chart-world container vertical-gradient shadow beveled default-border">
    ...
    <?php else :  // fips country code is not set ?>
       ...
       <?php require_once 'includes/tabs.php'; ?>
        ...
        <div id="map-filter">    
            <input type="text" value="country" class="map-search selectized" id="map-search" aria-label="Search or select a country" style="display: block;">
                <span id="label" class="title">Search or select a country</span>
        </div>
        <div id="world-map"></div>

<div id="most-populous2" class='full-border beveled'>
<br>
  ...
  <tr>
    <td valign=top width=33%>
      <table border=0 cellspacing=0 cellpadding=10 width=100%>
        <tr>
          <th class=primary_blue width=100%><b>Most Populous Countries</th>
        </tr>
        <?php 
    ...
    <td valign=top width=33%>
      <table border=0 cellspacing=0 cellpadding=10 width=100%>
        <tr>
          <th class=primary_blue width=100%><b>Top U.S. Export Partners</th>
        </tr>
        <tr>
          <td class=odd><span style="float:left;">1. <a href="./world/mx">Mexico</a></span><span style="float:right;">$338.0 B</span></td>
        </tr>
        <tr>
          <td class=even><span style="float:left;">2. <a href="./world/ca">Canada</a></span><span style="float:right;">$329.8 B</span></td>
        </tr>
        <tr>
          <td class=odd><span style="float:left;">3. <a href="./world/ch">China</a></span><span style="float:right;">$106.3 B</span></td>
        </tr>
        <tr>
          <td class=even><span style="float:left;">4. <a href="./world/uk">United Kingdom</a></span><span style="float:right;">$97.0 B</span></td>
        </tr>
        <tr>
          <td class=odd><span style="float:left;">5. <a href="./world/nl">Netherlands</a></span><span style="float:right;">$95.6 B</span></td>
        </tr>
      </table>
    </td>
    <td valign=top width=33%>
      <table border=0 cellspacing=0 cellpadding=10 width=100%>
        <tr>
          <th class=primary_blue width=100%><b>Top U.S. Import Partners</th>
        </tr>
        <tr>
          <td class=odd><span style="float:left;">1. <a href="./world/mx">Mexico</a></span><span style="float:right;">$534.9 B</span></td>
        </tr>
        <tr>
          <td class=even><span style="float:left;">2. <a href="./world/ca">Canada</a></span><span style="float:right;">$383.0 B</span></td>
        </tr>
        <tr>
          <td class=odd><span style="float:left;">3. <a href="./world/ch">China</a></span><span style="float:right;">$308.4 B</span></td>
        </tr>
        <tr>
          <td class=even><span style="float:left;">4. <a href="./world/tw">Taiwan</a></span><span style="float:right;">$201.4 B</span></td>
        </tr>
        <tr>
          <td class=odd><span style="float:left;">5. <a href="./world/vm">Vietnam</a></span><span style="float:right;">$193.8 B</span></td>
        </tr>
      </table>
    </td>
  </tr>
