<template>
  <div v-if="loading" class="widget-status">Loading highest density data…</div>
...
onMounted(async () => {
  try {
    const response = await fetch(apiUrl(API_ENDPOINTS.usDensity))
    const data = await response.json()

    if (!response.ok || !data.ok) {
      throw new Error(data.error || 'Failed to load highest density data.')
    }
